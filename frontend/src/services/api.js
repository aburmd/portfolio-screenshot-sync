...
// --- Zones ---
export async function fetchZones(market, symbol, basePos = 0.5, maxPos = 3.0, maxBuyZones = 5, maxSellZones = 5, hhTrimPct = 0.25, currentHoldingPct = 0.0) {
  const res = await fetch(`${API_BASE}/research/zones/${market}/${encodeURIComponent(symbol)}?base_pos=${basePos}&max_pos=${maxPos}&max_buy_zones=${maxBuyZones}&max_sell_zones=${maxSellZones}&hh_trim_pct=${hhTrimPct}&current_holding_pct=${currentHoldingPct}`);
