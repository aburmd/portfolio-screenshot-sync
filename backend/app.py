...
async def get_fundamentals(symbol: str, market: str = "US", period: str = "annual"):
    ...
    if inc is None or inc.empty:
        return {"error": f"No financial data for {yf_symbol}", "symbol": symbol}
    ...
    if fiscal_dates:
        ...
        try:
            hist_prices = yf.download(yf_symbol, start=price_start, end=price_end, progress=False, auto_adjust=True)
    ...
    meta_item = {
        ...
        "forward_pe": Decimal(str(round(info["forwardPE"], 2))) if info.get("forwardPE") else None,
        "trailing_pe": Decimal(str(round(info["trailingPE"], 2))) if info.get("trailingPE") else None,
        "operating_margin": Decimal(str(round(info["operatingMargins"], 4))) if info.get("operatingMargins") else None,
...
async def get_position_monitor(user_id: str, platform: str = None):
    ...
    for h in items:
        ...
        elif pnl_pct <= -20 and not is_quality and ma200 and price < ma200:
            signal = "SELL"
            reason = f"Down {pnl_pct:.0f}% + below 200MA + weak fundamentals \u2014 cut losses"
...
async def get_fractional_queue(user_id: str):
    try:
...
async def cancel_fractional_queue(item_id: str, user_id: str):
    try:
